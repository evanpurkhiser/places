...
```json
{
  "url": "https://instagram.com/some-post",
  "type": "instagram",
...
The `data` shape depends on the source type. For Instagram it can hold the post
payload; for a Google Maps import it can hold import and original-list context.
The example is illustrative rather than a provider payload contract.

## Relationship to places

A post can mention several places, and a place can be discovered through several
sources. Model this as a many-to-many relationship. A proposed `place_sources` join
table links place IDs to source IDs, with a unique constraint on each pair.
...
- Required fields, supported source types, and JSON validation per type.
- Source identity and deduplication, including URL normalization.
