...
export type Time =
  ...
  // A clock time on today's date in each place's time zone.
  | {kind: 'clock'; minuteOfDay: number}
  // Sunday 00:00 is minute 0 in each place's local recurring week.
...
function decodeTime(input: string): Time {
  ...
  if (match) {
    ...
    if (validHour && (meridiem || match[3] !== undefined)) {
      ...
      return {
        kind: 'weekly',
        minuteOfWeek: day * 1440 + minuteOfDay,
      };
    }
  }
...
export const time = valueType<Time, Context>({
  name: 'time',
  description:
